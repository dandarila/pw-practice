...
//test.describe.configure({mode: 'serial}); -> If one of the tests fails, all the other will be skipped 
test(' @pipeline Broswer Context-Validating Error login', async ({ page }) => {
...
customTest('Custom test fixture', async ({ page, testDataForOrder }) => {
    const poManager = new POManager(page);
	const text = ' Login Successfully ';
    const loginPage = poManager.getLoginPage();
    const dashboardPage = poManager.getDashboardPage();
